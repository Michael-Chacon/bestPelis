<div>
    <div class="grid grid-cols-2 md:grid-cols-1 gap-x-2">
        @forelse ($genres as $genero)
            <div class="bg-gradient-to-r from-red-900 via-black to-black text-white flex mb-2 justify-between items-center rounded-full py-1 px-4 font-medium border  border-white">
                ...
                <span class="hover:text-red-500">
                    <button wire:click="$emit('mostrarAlerta', {{ $genero->id }})">
        ...
        @empty
        <p class="flex capitalize text-center justify-center flex-1 p-5 bg-white text-black">No hay generos registrados</p>
        @endforelse
    </div>
</div>
...
@push('scripts')
    ...
    <script>
        Livewire.on('mostrarAlerta', generoId => {
            Swal.fire({
            ...
            if (result.isConfirmed) {
                    ...
                    Livewire.emit('eliminarGenero', generoId),
                    Swal.fire(
                    'Eliminada!',
